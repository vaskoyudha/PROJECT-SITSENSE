...
window.SidebarManager = {
  ...
  init: function () {
    ...
    if (savedState === 'true') {
      ...
      this.sidebar.querySelectorAll('.sidebar-title, .sidebar-subtitle, .sidebar-nav-text, .sidebar-logo').forEach(el => {
  ...
  markActiveNav: function () {
    ...
    const file = path.split('/').pop() || 'monitor.html';
        ...
        // Also check if current file matches the nav key's expected file
          ...
          switch (navKey) {
            case 'home':
              ...
              break;
            case 'dashboard':
              isActive = file === 'monitor.html' || file === '' || path.endsWith('/');
              break;
            case 'history':
